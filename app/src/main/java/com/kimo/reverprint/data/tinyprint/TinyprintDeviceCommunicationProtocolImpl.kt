package com.kimo.reverprint.data.tinyprint

import com.kimo.reverprint.data.tinyprint.TinyprintDeviceCommunicationProtocol.Mode
import com.lingmoyun.minilzo.MiniLZO
import kotlin.experimental.or

class TinyprintDeviceCommunicationProtocolImpl: TinyprintDeviceCommunicationProtocol {

    fun packLength(data: ByteArray): ByteArray = byteArrayOf(
        (data.size and 0xFF).toByte(),
        ((data.size shr 8) and 0xFF).toByte()
    )
    fun crc8(data: ByteArray): ByteArray {
        val crc8Table = intArrayOf(
            
        )
        var crc = 0
        for (byte in data) {
            val index = (crc xor (byte.toInt() and 0xFF)) and 0xFF
            crc = crc8Table[index]
        }
        return byteArrayOf((crc and 0xff).toByte())
    }
    fun formatMessage(cmd: Byte, data: ByteArray): ByteArray {
        return byteArrayOf(
            0x51.toByte(),
            0x78.toByte(),
            cmd,
            0x00.toByte(),
            *packLength(data),
            *data,
            *crc8(data),
            0xFF.toByte()
        )
    }
    fun compress(data: ByteArray): ByteArray {
        val compressedData = MiniLZO.compress(data)
        return byteArrayOf(
            *packLength(data),
            *packLength(compressedData),
            *compressedData
        )
    }

    override fun setMode(mode: Mode): ByteArray {
        val modeAsData = when (mode) {
            Mode.GREY_IMG -> byteArrayOf(0, 1)
            Mode.MONO_IMG -> byteArrayOf(0)
        }
        return formatMessage(0xBE.toByte(), modeAsData)
    }

    override fun setQuality(level: TinyprintDeviceCommunicationProtocol.Quality): ByteArray {
        val code = when(level) {
            TinyprintDeviceCommunicationProtocol.Quality.One -> 49.toByte()
            TinyprintDeviceCommunicationProtocol.Quality.Two -> 50.toByte()
            TinyprintDeviceCommunicationProtocol.Quality.Three -> 51.toByte()
            TinyprintDeviceCommunicationProtocol.Quality.Four -> 52.toByte()
            TinyprintDeviceCommunicationProtocol.Quality.Five -> 53.toByte()
        }
        return formatMessage(0xA4.toByte(), byteArrayOf(code))
    }

    override fun setEnergy(): ByteArray {
        TODO("Not yet implemented")
    }

    override fun feedPaper(lines: Int): ByteArray =
        formatMessage(0xA1.toByte(), byteArrayOf(lines.toByte()))

    override fun retractPaper(lines: Int): ByteArray =
        formatMessage(0xA0.toByte(), byteArrayOf(lines.toByte()))

    override fun println1bpp(bitmapLine: IntArray): ByteArray =
        formatMessage(0xA2.toByte(), pack1bpp(bitmapLine))

    override fun println4bpp(bitmapLine: IntArray): ByteArray =
        formatMessage(0xCF.toByte(), compress(pack4bpp(bitmapLine)))

    fun pack1bpp(pixels: IntArray): ByteArray {
        require(pixels.all { it == 0 || it == 1 })
        val result = ByteArray((pixels.size + 7) / 8)
        for (i in pixels.indices) {
            val byteIndex = i / 8
            val bitIndex = 7 - (i % 8)
            if (pixels[i] == 1)
                result[byteIndex] = result[byteIndex] or (1 shl bitIndex).toByte()
        }
        return result
    }

    fun pack4bpp(pixels: IntArray): ByteArray {
        val result = ByteArray((pixels.size + 1) / 2)

        for (currentByte in pixels.indices step 2) {
            val thisPixel = pixels[currentByte]
            val nextPixel = if (currentByte + 1 < pixels.size) pixels[currentByte + 1] and 0x0F else 0
            result[currentByte.coerceAtMost(result.lastIndex)] = ((thisPixel shl 4) or nextPixel).toByte()
        }

        return result
    }
}